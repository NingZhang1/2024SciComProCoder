subroutine BASIS_INT1(i,j,k,l,ncenters,nconts,coord,atomchg,S,Hcore,twoout,baselable)
implicit none
! input data zone
integer       :: ncenters,nconts
integer       :: atomchg(ncenters) ! natoms, multiplicity, charge
character     :: baselable*25
...
integer       :: i,j,k,l
real(8)       :: TWOEIout(1,1,1,1)
!!!add by LHC
integer :: kbasst(ncenters),kbased(ncenters),kcenter(ncenters)
real*8  :: twoout
!!!end
nRec = index_2e(nconts-1,nconts-1,nconts-1,nconts-1)+1
!print*,nconts,nRec
allocate(TWOEI(1))
call basisint1(i,j,k,l,ncenters,coord,atomchg,baselable,nconts,S,Hcore,nRec,twoout)
!write(*,*)'111',twoout,i,j,k,l
TWOEIout(i,j,k,l)  = twoout
!write(*,*)'222',TWOEIout(i,j,k,l),i,j,k,l
deallocate(TWOEI)
!open(21,file='one_electron_integral.txt')
!open(22,file='overlap_matrix.txt')
!do i=1,nconts
!       write(21,*)Hcore(i,:)
!       write(22,*)S(i,:)
!enddo
...
!===============================
subroutine basisint1(ti,tj,tk,tl,ncenters,coord,atomchg,baselable,n1,Sout,Hcoreout,n2,twoout)
...
use MOL_info
    ...
    character     :: baselable*25      ! basis lable, functional
    ...
    !!!add by LHC
    integer    :: kbasst(ncenters),kbased(ncenters),kcenter(ncenters)
    integer    :: num,si,sj,sk,sl,ii,jj,kk,ll,e1,e2,e3,e4
    real*8     :: intVal,twoout
    !!!end 
    ...
    filename=trim(adjustl(baselable))//'.bas'
...
do i = 1,Natoms
  ...
  write (atoms(i)%base,"(I0.2,A1,A25)")  atomchg(i),'-',baselable
enddo
!print *,atomchg(1)
!print *,atoms(1)%base
...
!  2  Prepare the data 
 ...
 do i = 1,nAtoms
    nConts = atoms(i)%nconts+nConts
     kcenter(i)=i
     if(i>=2)then
     kbasst(i)=1+kbased(i-1)
     endif
     kbased(i)=nconts
        !write(*,*)'nConts:',nConts
 enddo

 !write(*,*)kbasst,kcenter,kbased
 do i = 1,nAtoms
             ...
             do ii=1,di
                do jj=1,dj
                   do kk=1,dk
                      ...
                      e3=sk+kk-1
                      e4=sl+ll-1
                      !write(21,*)e1+1,e2+1,e3+1,e4+1
                      !if(e1==ti-1.and.e2==tj-1.and.e3==tk-1.and.e4==tl-1)then
                       if(INDEX_2E(e1,e2,e3,e4)==INDEX_2E(ti-1,tj-1,tk-1,tl-1))then
                       ! integral based on shell
                       !print*,shls
                       !print*,atm,nAtoms
                       !print*,bas,nBases
                       call cint2e_cart(buf2e, shls,atm,nAtoms,bas,nBases,env,0_8)
                       intVal=buf2e(ii,jj,kk,ll)*NorVEC(e1+1)*NorVEC(e2+1)*NorVEC(e3+1)*NorVEC(e4+1)  
                      ! stop 
...
Sout = S
!TWOEIout = TWOEI
twoout=TWOEI(INDEX_2E(ti-1,tj-1,tk-1,tl-1)+1)
!print*,twoout
...
end subroutine basisint1
!=======================
subroutine Normal1(iL,di,bas,nBasis,buf1e,Norfac,nConts)
...
!=======================
    ...
    integer function INDEX_2E1(i,j,k,l)
    implicit none
    integer  ::   i,j,k,l
    integer  ::   ij,kl
    ...
    if (ij > kl) then 
        INDEX_2E1 = (ij+1)*ij/2 + kl
    else
        INDEX_2E1 = (kl+1)*kl/2 + ij
    endif
end function 


!=================
subroutine  store2e1(shls,bas,buf2e,TWOEI,di,dj,dk,dl,nConts,nRec,nBases,Norfac)
...
!=========================
subroutine  store1e1(shls,di,dj,bas,nBases,buf1e,Norfac,nConts,S)
